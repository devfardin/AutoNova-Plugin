...
class RelatedCar
{
    ...
    public function relatedcar($post_id)
    {
        $terms = get_the_terms(get_the_ID(), 'car_brand');

        if (empty($terms) || is_wp_error($terms)) {
            return '<p>Sorry, no cars matched your criteria.</p>';
        }
        ...
        if (!$query->have_posts()): ?>
            <div class="no_car_found_message_wrap">
                <!-- add a red brand color crox svg icon -->
                ...
                <p class="no_car_found_message">Sorry, no cars matched your criteria.</p>
            </div>
        <?php else: ?>
